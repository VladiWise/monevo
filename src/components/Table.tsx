      ...
      <thead>
        <tr className="bg-white even:bg-gray-100 z-10">
          {columns?.map((column, index) => (
            <th
              key={index}
              className="bg-gray-200 py-2 pl-3 pr-2 top-0 z-10 sticky text-left text-gray-500 text-sm"
            >
              {column.title}
            </th>
          ))}
          {actions && (
            <th className="bg-gray-200 py-2 pl-3 pr-2 top-0 z-10 sticky text-left text-gray-500 text-sm">
              Actions
            </th>
          )}
        </tr>
