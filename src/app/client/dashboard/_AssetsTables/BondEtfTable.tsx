...
interface BondEtfTableProps {
  bankAccounts: any;
  findAmountById: (item: any, accountId: string) => number;
  getDataByField: (moexJson: MoexJson, field: string) => any;
}

export async function BondEtfTable({
  bankAccounts,
  findAmountById,
  getDataByField,
}: BondEtfTableProps) {
  const currentUser = await getCurrentUser();
  const serverItems = await fundBService.getList(currentUser?.id);
  ...
  const serviceEtf = {
    ...
    create: fundBService.create,
    updateAccounts: fundBService.updateAccounts,
    remove: fundBService.remove,
  };
